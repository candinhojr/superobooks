...
const CellBooks = ({ cellkey, data, column }) => {
    ...
    return (
        <TableCell><Typography variant='body2'>{data[column.id]}</Typography></TableCell>
    )
}
    ...
    return (
        <TableContainer component={Paper}>
            <Table className={classes.table} size="small" aria-label="a dense table">
                <TableHead>
                    <StyledTableRow>
                        {columns.map(column => <StyledTableCell key={column.id} align={column.align}>{column.label}</StyledTableCell>)}
                ...
                <TableBody>
                    {(rowsPerPage > 0
                        ...
                        <StyledTableRow key={data.id}>
                            {columns.map(column => (<CellBooks key={column.id} data={data} column={column} />))}
                    ...
                    {emptyRows > 0 && (
                        <TableRow style={{ height: 10 }}>
                            <TableCell colSpan={5}> <Typography variant='body2'>Não foram encontrados registros</Typography></TableCell>
                        </TableRow>
                    )}
                </TableBody>
                <TableFooter>
